...
memory_block*My_heap::first_fit_allocate(int num_bytes)
{
	...
	else
	{
		...
		else
		{
			...
			while (!(ptr->used == false && ptr->size >= num_bytes))
			{
				ptr=ptr->right;
...
memory_block*My_heap::best_fit_allocate(int num_bytes)
{
	...
	else
	{
		...
		tracker = heap_begin;
		while (!(tracker->used == false && tracker->size >= num_bytes))
		{
			tracker = tracker->right;
	...
	else
	{
		...
		if (found == true) // if we found it
		{
			...
			else
			{
				...
				newfreeblk->finishing_adress = newfreeblk->starting_address + (ptr->size - num_bytes);
...
void My_heap::deallocate(memory_block* block_address)
{
	...
	if (block_address->left->used == false && block_address->right == false) // in case if block is in between two used=false blocks.
	{
